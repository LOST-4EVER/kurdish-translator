...
const Translator = (() => {
  ...
  function preprocessSource(text, srcLang, tgtLang) {
    ...
    s = s.replace(/\bgonna\b/gi, 'going to')
         ...
         .replace(/\bget\s+out\s+of\s+here\b/gi, 'leave right now')
          ...
          .replace(/\bfire\s+in\s+the\s+hole\b/gi, 'danger explosive')
          .replace(/\block\s+and\s+load\b/gi, 'prepare weapons')
          .replace(/\bcode\s+red\b/gi, 'high emergency')
          .replace(/\ball\s+clear\b/gi, 'all safe')
          .replace(/\bcease\s*fire\b/gi, 'stop shooting')
          .replace(/\bon\s+my\s+mark\b/gi, 'on my signal')
          .replace(/\babort\s+mission\b/gi, 'cancel mission')
          .replace(/\bcover\s+me\b/gi, 'protect me')
          .replace(/\bwatch\s+your\s+back\b/gi, 'be careful behind you')
          .replace(/\b(?:i['’]?ve\s+)?got\s+your\s+back\b/gi, 'I will protect you')
          .replace(/\bdrop\s+your\s+weapons?\b/gi, 'put down your weapon')
          .replace(/\bfall\s+back\b/gi, 'retreat now')
          .replace(/\bkeep\s+moving\b/gi, 'continue moving')
  ...
  /**
   * Fast server-side proxy translation when available (runs in Node / Cloud Run backend).
   */
  async function fetchServerProxy(text, srcLang, tgtLang, signal) {
    ...
    try {
      ...
      // Fall through to client-side direct endpoints
    } finally {
      scoped.cleanup();
    }
    return null;
  ...
  /**
   * Fetch from Google Translate lightweight /t endpoint (Highest stability, supports GET).
   */
  ...
  /**
   * Fetch from Google Translate web endpoints (/single) with direct GET support.
   */
  ...
  /** Translate one chunk with resilient multi-provider failover (/api/translate -> Google /single -> Google /t -> Lingva -> MyMemory). */
  async function translateChunk(text, srcLang, tgtLang, signal) {
    let lastErr;

    // Fast-path: try local/server-side /api/translate proxy if available
    try {
      ...
      try {
        if (attempt < 3) {
          ...
          try {
            ...
            try {
              return await fetchGoogle(text, srcLang, tgtLang, signal, 0);
            } catch {}
          }
        }
      } catch (err) {
